...
function renderSpec(){
  ...
  // Collection-level wild photos are intentionally shown on BN1 specimen pages without
  // falsely assigning them to a specimen. The photos remain stored once on the archive record.
  const archiveRecord=data.find(x=>x.code===`${s.collection}-DRIVE-ARCHIVE`);
  const collectionWild=(s.code!==`${s.collection}-DRIVE-ARCHIVE` && archiveRecord)
    ? (archiveRecord.images||[]).filter(im=>im.category==='Wild / field context' && (imageType==='ALL'||imageType==='Wild / field context'))
    : [];
  const collectionWildGallery=collectionWild.length?`<section class="section collection-archive"><h2>${esc(s.collection)} wild field archive — unassigned <span>${collectionWild.length}</span></h2><div class="sourceLine"><strong>Collection-level provenance</strong><p>These photographs belong to the ${esc(s.collection)} Drive collection but are not yet assigned to this specimen accession. They are shown here for comparison only; no specimen match is being inferred.</p></div><div class="gallery">${collectionWild.map(im=>`<article class="image-card"><div class="image-wrap" data-file="${esc(im.original)}" data-caption="${esc(im.caption)}" data-name="${esc(im.filename)}"><img loading="lazy" src="${esc(im.thumb)}" alt="${esc(im.caption)}"><span class="zoom-badge">Tap / click to inspect</span></div><div class="caption"><strong>${esc(im.caption)}</strong><code>${esc(im.filename)}</code><small class="drive-source">Collection-level image — not assigned to ${esc(s.code)}</small></div></article>`).join('')}</div></section>`:'';

  const eyebrow=s.archiveIndex?`MASTER RECORD ${s.archiveIndex} / ${masterCount}`:'DRIVE ARCHIVE';
  $('main').innerHTML=`<div class="spec-head"><div><div class="eyebrow">${eyebrow}</div><h1>${esc(s.code)}</h1><p class="status">${esc(s.status)}</p></div><div class="navBtns"><button onclick="nav(-1)">← Previous</button><button onclick="nav(1)">Next →</button></div></div><div class="hero"><div class="summary-grid">${sum}</div><aside class="card"><h3>Data completeness</h3><div class="matrix"><div>Wild + lab pairing</div><div class="${pairClass}">${pairStatus}</div>${mat}</div>${s.sourceLine?`<div class="sourceLine"><strong>Source / provenance</strong><p>${esc(s.sourceLine)}</p></div>`:''}</aside></div>${galleries||''}${collectionWildGallery||''}${movieGallery||''}${(!galleries&&!collectionWildGallery&&!movieGallery)?'<div class="empty">No media match the selected filter.</div>':''}`;
